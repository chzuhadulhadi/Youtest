...
module.exports = async function deleteMailingListUser(req, res) {
	...
	try {
		...
		if (validate.error) {
			res.status(StatusCodes.BAD_REQUEST).send({
				data: {},
				message: translation["deleteMailingList"][language].errorMessage,
				error: err.stack,
			});
		}
		const data = await mailingListUserModel.deleteMailingListUser(validate);
		res
